...
import type { Tarea } from "./components/TareasList/TareasList";
import { getTareas, /*crearTarea, actualizarTarea, eliminarTarea*/ } from "./components/TareasList/TareasList";
...
function App() {
  ...
  return (
    <ul>
      {tareas.map((t) => (
        <li key={t.id}>{t.titulo}, {t.descripcion}, {t.fechaCreacion} — {t.estado}</li>
      ))}
    </ul>
  );
}
